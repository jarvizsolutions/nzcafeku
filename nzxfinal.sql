-- ============================================================
-- nzxfinal.sql — COMPLETE consolidated schema + every incremental
-- Run this single file in a brand-new Supabase project's SQL Editor.
-- (It is: MIGRATE.sql followed by nz.sql, glued together.)
-- ============================================================
...
-- ============================================================
-- BEGIN INCREMENTAL CHANGES (from nz.sql)
-- ============================================================

-- ============================================================
-- nz.sql — incremental migration for the changes added in this batch
-- Run this in your own Supabase project (SQL Editor) AFTER MIGRATE.sql.
-- Safe to re-run (idempotent).
-- ============================================================

-- 1) Add customer_phone column to orders
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS customer_phone text;

-- 2) Recreate append_order_items with _customer_phone parameter
DROP FUNCTION IF EXISTS public.append_order_items(integer, uuid, text, text, numeric, jsonb, text);
DROP FUNCTION IF EXISTS public.append_order_items(integer, uuid, text, text, text, numeric, jsonb, text);
...
BEGIN
  ...
  IF v_session_id IS NOT NULL THEN
    SELECT id INTO v_order_id FROM public.orders
    WHERE session_id = v_session_id AND is_paid = false
      AND status IN ('pending','preparing','ready')
      AND lower(regexp_replace(btrim(coalesce(customer_name, '')), '\s+', ' ', 'g')) = v_name_key
    ORDER BY created_at DESC LIMIT 1 FOR UPDATE;
  END IF;
  ...
  IF v_order_id IS NULL AND v_session_id IS NULL THEN
    SELECT id INTO v_order_id FROM public.orders
    WHERE table_number = _table_number AND session_id IS NULL AND is_paid = false
      AND status IN ('pending','preparing','ready')
      AND lower(regexp_replace(btrim(coalesce(customer_name, '')), '\s+', ' ', 'g')) = v_name_key
    ORDER BY created_at DESC LIMIT 1 FOR UPDATE;
  END IF;
  ...
  UPDATE public.orders
     SET total = v_new_total, last_appended_at = now(), updated_at = now(),
         idempotency_key = _idempotency_key,
         customer_name = v_name,
...
-- 3) Recreate waiter_place_order with _customer_phone parameter
DROP FUNCTION IF EXISTS public.waiter_place_order(integer, text, text, jsonb, text);
DROP FUNCTION IF EXISTS public.waiter_place_order(integer, text, text, text, jsonb, text);
...
BEGIN
  ...
  UPDATE public.orders
     SET total = v_total, last_appended_at = now(), updated_at = now(),
         idempotency_key = _idempotency_key,
         customer_name = v_name,
         customer_phone = COALESCE(v_phone, customer_phone)
   WHERE id = v_order_id;
...
-- Done. All UI changes are pure frontend and need no further SQL.

-- =====================================================================
-- 2026-05-27: Item-tick + manual served behavior
-- =====================================================================

-- Per-item prepared progress counter (e.g. 2 of 3 chickens done)
ALTER TABLE public.order_items
...
-- Tick / un-tick one unit (or any delta) of an item's prepared quantity.
...
-- Legacy set_item_prepared: keep it working, but stop auto-serving.
CREATE OR REPLACE FUNCTION public.set_item_prepared(_item_id uuid, _prepared boolean)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- =====================================================================
-- 2026-05-27 (later): per-item cancellation by kitchen + realtime hardening
-- =====================================================================
...
-- Cancel a single line item; recompute the order total excluding cancelled items.
...
-- Make realtime UPDATE payloads include all columns so customers see live ticks/cancellations.
ALTER TABLE public.orders REPLICA IDENTITY FULL;
ALTER TABLE public.order_items REPLICA IDENTITY FULL;
...
-- Refresh PostgREST schema cache so newly created functions are immediately callable.
NOTIFY pgrst, 'reload schema';

-- =====================================================================
-- 2026-05-27 (later): Force PostgREST to re-read functions
-- Fixes: "Could not find the function public.cancel_order_item(_item_id)"
-- Re-declare to bump the schema cache, then notify.
-- =====================================================================
...
-- =====================================================================
-- 2026-05-27 (later): Menu item priority / name edit
-- No schema change — the `sort_order` column already exists on menu_items
-- and the customer Menu page already orders by it. The admin UI now lets
-- staff edit `name` and `sort_order` per dish. Optional: backfill a sane
-- default so unsorted items appear after sorted ones.
-- =====================================================================
...
UPDATE public.menu_items
 ...
 WHERE public.menu_items.id = sub.id
   AND (public.menu_items.sort_order IS NULL OR public.menu_items.sort_order = 0);

NOTIFY pgrst, 'reload schema';

-- =====================================================================
-- 2026-05-29: Pro Admin "Reset Data" RPC
-- Wipes orders / items / sessions / waiter calls / feedback / monthly reports.
-- Keeps menu, categories, combos, tables, announcements, users, settings.
-- =====================================================================
